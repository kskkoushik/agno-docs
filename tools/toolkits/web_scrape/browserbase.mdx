...
Note: `connect_url` parameter is now included in several functions to specify the connection URL of an existing session, allowing better session management and operation distinction.